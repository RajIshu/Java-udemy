...
public class NumberPalindrome {
    ...
    public static boolean isPalindrome (int number){
        int anotherNumber = Math.abs(number);
        // The method Math. abs() converts the provided negative integer to a positive integer.

        int comparisonNumber = 0;

        while (anotherNumber > 0){
            comparisonNumber += anotherNumber % 10;
            // System.out.println("Comparison Number = " + comparisonNumber);
            comparisonNumber *= 10;
            anotherNumber /= 10;
        ...
        if(comparisonNumber / 10 == Math.abs(number)){
            return true;
        }
        return false;
    }
