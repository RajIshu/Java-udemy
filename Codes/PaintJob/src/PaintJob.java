...
Bob calculate how many buckets of paint he needs to buy before going to work. Bob might
also have some extra buckets at home. He also knows the area that he can cover with one
bucket of paint.
...
1. Write a method named getBucketCount with 4 parameters. The first parameter should be
   ...
   that Bob needs to buy before going to work. To calculate the bucket count, refer to the
   notes below.
...
2. Bob does not like to enter 0 for the extraBuckets parameter, so he needs another method.
   ...
   If one of the parameters' width, height or areaPerBucket is less or equal to 0, the
   ...
   * getBucketCount(-3.4, 2.1, 1.5); → should return -1 since the width parameter is
     invalid
...
3. In some cases, Bob does not know the width and height of the wall, but he knows the
   area of a wall. He needs you to write another method.

   Write another overloaded method named getBucketCount with 2 parameters namely, area
   and areaPerBucket (both of type double).

   The method needs to return a value of type int that represents the number of buckets
   that Bob needs to buy before going to work. To calculate the bucket count, refer to
   the notes below.

   If one of the parameters area or areaPerBucket is less or equal to 0, the method
   needs to return -1to indicate an invalid value.
   ...
   * getBucketCount(3.26, 0.75); → should return 5 since the wall area is 3.26, and
     a single bucket can cover an area of 0.75 .
